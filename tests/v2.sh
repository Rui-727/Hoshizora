...
rm -f /tmp/hz_v2_listen.sock /tmp/hz_v2_marker
mkdir -p "$SOCKDIR"
trap 'rm -rf "$SOCKDIR" /tmp/hz_v2_listen.sock /tmp/hz_v2_marker; kill $PID 2>/dev/null || true' EXIT

HZ_CTL_PATH="$SOCK" HZ_NOTIFY_PATH="$SOCKDIR/notify" $HZ tests/v2.hs > /tmp/hz_v2.log 2>&1 &
PID=$!
...
FAIL=0
chk() {
    if grep -q "$1" /tmp/hz_v2.log; then
        echo "PASS: $2"
        PASS=$((PASS+1))
    else
        echo "FAIL: $2"
...
if [ "$(grep -c 'started quitter' /tmp/hz_v2.log)" -ge 2 ] && [ "$(grep -c 'started listener' /tmp/hz_v2.log)" -ge 2 ]; then
...
if grep -qE "dropped capabilities|capset:.*running with full caps" /tmp/hz_v2.log; then
...
if grep -q "notify socket:" /tmp/hz_v2.log; then
...
# No fatal errors
if grep -qE "parse error|FATAL|Assertion" /tmp/hz_v2.log; then
    echo "FAIL: errors in log"
    FAIL=$((FAIL+1))
fi

rm -f /tmp/hz_v2.log
echo "--- v2 self-check: $PASS PASS, $FAIL FAIL ---"
[ "$FAIL" -eq 0 ] || exit 1
