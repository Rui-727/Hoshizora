...
# ponytail: one config, one assertion path — service crashes past max_restarts,
...
HZ_CTL_PATH="$SOCK" $HZ tests/onfail.hs > /tmp/hz_onfail.log 2>&1 &
PID=$!
...
if kill -0 $PID 2>/dev/null; then
    ...
    wait 2>/dev/null
    cat /tmp/hz_onfail.log
    rm -f /tmp/hz_onfail.log
    exit 1
fi
wait $PID 2>/dev/null
...
FAIL=0
chk() {
    if grep -q "$1" /tmp/hz_onfail.log; then
        echo "PASS: $2"
        PASS=$((PASS+1))
    else
        echo "FAIL: $2"
...
# as a test failure.
if grep -qE "parse error|FATAL|Assertion" /tmp/hz_onfail.log; then
    echo "FAIL: errors in log"
    FAIL=$((FAIL+1))
fi
rm -f /tmp/hz_onfail.log

echo "--- on-fail self-check: $PASS PASS, $FAIL FAIL ---"
[ "$FAIL" -eq 0 ] || exit 1
