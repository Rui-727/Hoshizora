...
New here? Read [`GUIDE.md`](GUIDE.md) first. It walks through testing
Hoshizora safely, from `make test` (sandbox) to QEMU boot to real hardware
install.

Testing safely: Levels 1 (sandbox), 2 (user namespace), and 4 (LFS in QEMU)
are zero-risk on any host. Level 5 (real hardware) requires a tested recovery
plan. Don't do it on a box you can't afford to lose. LFS in QEMU gives the
full real-PID-1 experience without boot risk.
